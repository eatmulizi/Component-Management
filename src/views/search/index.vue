<template>
    ...
    <div class="form">
      <el-form ref="form" :model="Form" label-width="180px">
        ...
        <el-form-item label="供应商名字">
          <el-input v-model="searchForm.supplyName"></el-input>
        </el-form-item>
    

        <el-form-item label="供应商城市">
          <el-input v-model="searchForm.supplyCity"></el-input>
        </el-form-item>
        <el-form-item label="供应商闲鱼号">
          <el-input v-model="searchForm.supplyFish"></el-input>
        </el-form-item>
  ...
  <div>
    ...
    <!-- 图片展示区域 -->
    <div>
      <!-- 这里踩坑了需要隔离div 不然样式会统一 -->
    <div v-for="(imageSrc, index) in paginatedImages" :key="index" class="image-container">
      <el-card class="image-card" :body-style="{ padding: '0px' }">
        <img :src="imageSrc" alt="Base64 Image" :style="{ width:'200px', maxHeight:'200px' }" @click="handlePictureCardPreview(imageSrc)" />
      </el-card>
    </div>
...
export default {
  ...
  computed: {
    paginatedImages() {
      ...
      return this.imageSrc.slice(start, end);
    }
  },
  methods: {
    // 图片的分页写法
    ...
    handlePictureCardPreview(imageSrc) {
      //图片预览
      this.dialogImageUrl = imageSrc;
    ...
    newSearch() {
      ...
      this.imageSrc = []
      var form = {}
      if (this.searchForm.supplyName) form.supplyName = this.searchForm.supplyName;
      if (this.searchForm.supplyCity) form.supplyCity = this.searchForm.supplyCity;
      if (this.searchForm.supplyFish) form.supplyFish = this.searchForm.supplyFish;
